const ListingType = require("../models/listingtype.model");
...
class ListingTypecontroller {
  ...
  async Create(req, res) {
    try {
      ...
      const created = await ListingType.create({ type });
      if (!created) {
        return res
          .status(400)
  ...
  async GetAll(req,res){
    try{
        const get=await ListingType.find();
        if(!get){
  ...
  async GetById(req,res){
    try{
        const {id}=req.params;
        const found=await ListingType.findById(id);
        if(!found){
...
module.exports=new ListingTypecontroller();